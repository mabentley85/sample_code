...
class SchoolController extends Controller
{
	...
	{
		$softDelete = School::model()->findByPk($id);
	...
	public function actionIndex()
	{
		...
		{
			$dataProvider=new CActiveDataProvider('School',	array('criteria'=>array('condition'=>'isDeleted=0'),));
		}
		elseif (Yii::app()->user->checkAccess("district_admin"))
		{
			$userDistrictId = User::model()->findByPk(Yii::app()->user->id)->district_id;
			$dataProvider = new CActiveDataProvider('School', array('criteria'=>array('condition'=>'isDeleted=0 && district_id=' . $userDistrictId)));	
		}
		elseif (Yii::app()->user->checkAccess("teacher"))
		{
			$userSchool = User::model()->findByPk(Yii::app()->user->id)->school_id;
			$dataProvider = new CActiveDataProvider('School', array('criteria'=>array('condition'=>'isDeleted=0 && id=' . $userSchool)));	
		}
